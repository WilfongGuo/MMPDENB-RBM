function [OffMask,poss_s_num]= Operator1(ParentMask,rbm,Site,allZero,allOne,D_score,test_adjacency)
...
%         OffTemp = BinaryCrossover(rbm.reduce(Parent1Mask(Site,other)),rbm.reduce(Parent2Mask(Site,other)));
        ...
        OffMask = false(size(OffTemp,1),size(ParentMask,2));
        OffMask(:,other)  = OffTemp;
        OffMask(:,allOne) = true;
    else
        OffMask = [];
    end
    OffMask = [OffMask;Cross1(sum(~Site),size(ParentMask,2),ParentMask)];
